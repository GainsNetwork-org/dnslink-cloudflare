...
async function getRuleIds(api, zone, rulesetId, record) {
  const res = await api(`zones/${zone}/rulesets/${rulesetId}`);
  const ruleIds = [];

  for (const rule of res.body.result.rules) {
    if (rule.description === `[${record}] Redirect to /ipfs/<CID>`) {
      ruleIds.push(rule.id);
    } else if (rule.description === `[${record}] Redirect non-html routes to .html`) {
      ruleIds.push(rule.id);
    }
  }
...
async function getDnsLinkRecord(api, zone, name) {
  ...
  for (
    ...
    for (const record of res.body.result) {
      const normalizedContent = record.content.replace(/^"|"$/g, '');
      if (record.name === name && normalizedContent.startsWith("dnslink=")) {
        return record;
      }
    }
  }
...
async function update(apiOpts, { zone, link, record }) {
  ...
  const [ruleId1, ruleId2] = await getRuleIds(api, zoneId, rulesetId, record);
  console.log(`Found ruleIds ${ruleId1} and ${ruleId2}`);

  if (!ruleId1 || !ruleId2) {
    throw new Error("Couldn't find the rules");
  }

  const dnsLink = { content: `dnslink=${link}`, name: `_dnslink.ipns.${record}` };
  ...
  const promises = [
    api.patch(`zones/${zoneId}/rulesets/${rulesetId}/rules/${ruleId1}`, {
      json: {
        ...
        description: `[${record}] Redirect to /ipfs/<CID>`,
      },
    }),
    api.patch(`zones/${zoneId}/rulesets/${rulesetId}/rules/${ruleId2}`, {
      json: {
        ...
        description: `[${record}] Redirect non-html routes to .html`,
      },
    }),
    dnsLinkRecord

...
async function update (apiOpts, { zone, link, record }) {
  ...
  const ruleIds = await getRuleIds(api, zoneId, rulesetId);

  const test = await api.get(`zones/${zoneId}/rulesets/${rulesetId}`);

  await Promise.all([
    api.patch(`zones/${zoneId}/rulesets/${rulesetId}/rules/${ruleIds[0]}`, {
      json: {
        "action": "rewrite",
        "expression": "(http.host eq \"ipfs.gains.trade\")",
        "action_parameters": {
          "uri": {
            "path": { 
    ...
    api.patch(`zones/${zoneId}/rulesets/${rulesetId}/rules/${ruleIds[1]}`, {
      json: {
        "action": "rewrite",
        "expression": "(http.host eq \"ipfs.gains.trade\" and http.request.uri ne \"/\" and not http.request.uri contains \".\")",
        "action_parameters": {
          "uri": {
            "path": { 

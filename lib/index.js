...
async function getDnsLinkRecord (api, zone, name) {
  ...
  for (let i = 1; (res = await api(`zones/${zone}/dns_records?type=TXT&page=${i}`)) && res.body.result_info.total_pages >= i; i++) {
...
async function update (apiOpts, { zone, link, record }) {
  ...
  console.log(dnsLinkRecord ? `Found dnsLink ${dnsLinkRecord.id}` : 'No dnsLink record found. A new TXT record will be created.');

  const promises = [
    api.patch(`zones/${zoneId}/rulesets/${rulesetId}/rules/${ruleId1}`, {
      json: {
        "action": "rewrite",
        "expression": "(http.host eq \"" + record + "\" or http.host eq \"staging." + record + "\")",
        "action_parameters": {
          "uri": {
            "path": {
    ...
    api.patch(`zones/${zoneId}/rulesets/${rulesetId}/rules/${ruleId2}`, {
      json: {
        "action": "rewrite",
        "expression": "((http.host eq \"" + record + "\" or http.host eq \"staging." + record + "\") and http.request.uri.path ne \"/\" and not http.request.uri.path contains \".\")",
        "action_parameters": {
          "uri": {
            "path": {
